...
const addFavouriteToLocalStorage = (favouriteToAdd) => {
  ...
  ); // Cuando añado un favorito, guardo el array de favoritos en localStorage
};
...
const removeFavouriteToLocalStorage = (favouriteToRemove) => {
  ...
  localStorage.setItem(
    favouritesOnLocalStorage,
    JSON.stringify(favouritesListStorage)
  ); // Cuando elimino un favorito, vuelvo a guardar el array de favoritos en localStorage
  counter--;
...
const removeFavourite = (liHtml) => {
  ...
  const indexFavouriteToRemove = favourites.findIndex(
    (favourite) => favourite.id === liHtml.id
  );
  favourites.splice(indexFavouriteToRemove, 1);
...
const addFavourite = (liHtml) => {
  ...
  const indexOnFavourites = Array.from(favouritesList.childNodes).findIndex(
    (favourite) => favourite.id === liHtml.id
  );
